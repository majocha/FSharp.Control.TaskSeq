...
type RuntimeTaskBuilder() =
    ...
    member inline _.Combine([<InlineIfLambda>] first, [<InlineIfLambda>] second) =
        first()
        second()
...
module MediumPriority =
    type RuntimeTaskBuilder with  
        member inline _.Bind(code: struct ('T1 * 'T2), [<InlineIfLambda>] continuation: struct ('T1 * 'T2) -> 'U) =
            continuation code
        member inline _.Bind([<InlineIfLambda>] computation: RuntimeTask<'T>, [<InlineIfLambda>] continuation: 'T -> 'U) =
